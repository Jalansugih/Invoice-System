// CATATAN (audit fullstack): fungsi createInvoiceAction/calculateInvoiceAmounts
// di file ini TIDAK dipanggil dari komponen UI manapun — alur simpan invoice
// yang aktif adalah StorageService.saveInvoice() di src/lib/storage.ts.
// Proteksi "harga tidak bisa dimanipulasi" yang dimaksud file ini sekarang
// ditegakkan di dua tempat yang benar-benar berjalan:
//   1. StorageService.saveInvoice() — recompute qty*harga-diskon & timpa
//      harga dari produk master sebelum disimpan (lihat invoiceCalc.ts).
//   2. Trigger database di supabase/migration_v22_invoice_integrity.sql —
//      menegakkan ulang hal yang sama di level Postgres, terlepas dari
//      jalur mana yang menulis ke tabel invoices/invoice_items.
// File ini dibiarkan ada sebagai referensi/dokumentasi rumus, tapi jangan
// diasumsikan berjalan sampai benar-benar di-wire ke sebuah endpoint.
