...
export async function createInvoiceAction(
  ...
  try {
    ...
    // 5. Persist through StorageService, which is the hardened persistence
    // boundary. Its create path uses create_invoice_atomic() when Supabase is
    // configured, so this action does not perform independent DB writes.
